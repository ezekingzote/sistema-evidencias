                                ...
                                <div class="table-responsive">
                                    <table class="table table-borderless align-middle" id="tablaPonderaciones">
                                        ...
                                        <tbody id="tbodyInstrumentacion">
                                            ...
                                            @forelse ($ponderaciones as $pond)
                                                <tr class="border-bottom fila-criterio">
                                                    <td class="ps-3">
                                                        <input type="text" name="actividades[]" class="form-control form-control-sm border-0 bg-light" 
                                                               value="{{ $pond->actividad }}" {{ isset($modo) && $modo == 'ver' ? 'readonly' : '' }} required>
                                                    </td>
                                                    <td>
                                                        <div class="input-group input-group-sm w-75 mx-auto">
                                                            <input type="number" name="porcentajes[]" class="form-control border-0 bg-light text-center fw-bold input-porcentaje" 
                                                                   min="1" max="100" value="{{ $pond->porcentaje }}" {{ isset($modo) && $modo == 'ver' ? 'readonly' : '' }} required>
                                                            <span class="input-group-text border-0 bg-light small">%</span>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <input type="text" name="instrumentos[]" class="form-control form-control-sm border-0 bg-light" 
                                                               value="{{ $pond->instrumento }}" {{ isset($modo) && $modo == 'ver' ? 'readonly' : '' }} required>
                                                    </td>
                                                    <td class="text-center">
                                                        @if (isset($modo) && $modo == 'ver')
                                                            <span class="text-muted small" title="Solo lectura"><i class="bi bi-lock-fill"></i></span>
                                                        @else
                            ...
                            <div class="card-footer bg-light border-0 p-4">
                                <div class="row align-items-center">
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <span id="mensajePorcentaje"
                                                    ...
                                                    <i class="bi bi-exclamation-triangle-fill me-1"></i> Falta 100% para el total
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6 text-md-end mt-3 mt-md-0">
                                        @if (isset($modo) && $modo == 'ver')
                                            <a href="{{ route('planes-estudio') }}" class="btn btn-primary px-4 shadow text-white">
                                                <i class="bi bi-arrow-left me-1"></i> Volver a Planes
                                            </a>
                                        @else
                                            <a href="{{ route('planes-estudio') }}" class="btn btn-outline-danger me-2">Cancelar</a>
                                            <button type="submit" id="btnGuardar" class="btn btn-outline-success px-4 shadow" disabled>
                                                Guardar Ponderación
                                            </button>
                                        @endif
                                    </div>
                ...
                btnAgregar.addEventListener('click', function() {
                    const nuevaFila = `
                        <tr class="border-bottom fila-criterio">
                            ...
                            <td>
                                <div class="input-group input-group-sm w-75 mx-auto">
                                    <input type="number" name="porcentajes[]" class="form-control border-0 bg-light text-center fw-bold input-porcentaje" min="0" max="100" value="0" required>
                                    <span class="input-group-text border-0 bg-light small">%</span>
                                </div>
                            </td>
                            <td>
